 ...
 * and corner radius it currently renders with. A pick is seeded with the
 * toolbar's settings at click time; afterwards only elements in the active
 * editing set (see `active`) follow live setting changes, so any other entry
 * keeps the values it was last given. */
...
export class OverlayController {
  ...
  // The "active" editing set: the element(s) most recently selected — a single
  // plain click, or the group built with Shift+click. Live setting changes
  ...
  /** Live-apply per-element settings to every element in the active editing set
   * (the last plain-clicked element, or the Shift+click group), leaving all
   * other spotlit elements frozen. Called by the toolbar as sliders / the color
   * swatch change so the current selection updates as you tweak it. */
  applyToActive(
    patch: Partial<
      Pick<SelectionEntry, 'color' | 'padding' | 'outlineWidth' | 'radius'>
    >,
  ): void {
    ...
    for (const entry of this.selected) {
      if (this.activeEls.includes(entry.el)) {
  ...
  /** Wipe the selection AND the undo history in one shot, with nothing to step
   * back to afterward (unlike clearSelection, which records an undo step). Used
   * by the toolbar's "Clear all history" reset. */
  clearAll(): void {
    this.selected.length = 0;
    this.activeEls = [];
    this.history.length = 0;
  ...
  /** Record the current selection in the undo history without changing it. The
   * toolbar calls this right before a live settings edit (a slider stop, a
   * color pick) so Undo can step back through per-element setting changes, not
   * just pick/unpick/clear. A no-op when nothing is being edited, so adjusting
   * settings with no active selection never piles up empty undo steps. */
  ...
  // Swallow the page's default mousedown while picking so a click — especially a
  // Shift+click — never starts a text selection or shifts focus underneath.
  ...
  private onPointerClick(event: MouseEvent): void {
    ...
    // Shift extends the active editing group; a plain click starts a fresh one.
    this.toggleSelect(target, event.shiftKey);
  }
  ...
  private toggleSelect(el: Element, additive: boolean): void {
    this.pushHistory();
    ...
    if (additive) {
      if (removed) {
        this.activeEls = this.activeEls.filter((a) => a !== el);
      } else if (!this.activeEls.includes(el)) {
        this.activeEls.push(el);
      }
    } else {
      this.activeEls = removed ? [] : [el];
    }
    // Drop any active refs that are no longer spotlit.
