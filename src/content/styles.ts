...
/* Footer row: Hotkey + Clear history + Reset, split evenly across one row. */
.nhost-ss-set-footer { margin-top: 2px; }
.nhost-ss-set-footer > .nhost-ss-set-reset { flex: 1 1 0; padding: 3px 8px; }

/* "Clear history" — a destructive-tinted reset sharing the footer row. */
.nhost-ss-set-reset.nhost-ss-set-clear:hover {
...
.nhost-ss-slider:focus,
.nhost-ss-slider:focus-visible { outline: none; }
...
.nhost-ss-crop-rect {
  ...
  border-radius: 4px;
  cursor: grab;
  /* Marching ants: a 1px dashed line on each edge (light/dark two-tone so it
   * reads on any screenshot), animated so the dashes crawl around the box. Each
   * gradient is sized to a single edge and repeated along that edge only, so
   * the interior stays clear. */
  background-image:
    repeating-linear-gradient(90deg, #fff 0 6px, #111 6px 12px),
    repeating-linear-gradient(90deg, #fff 0 6px, #111 6px 12px),
    repeating-linear-gradient(0deg, #fff 0 6px, #111 6px 12px),
    repeating-linear-gradient(0deg, #fff 0 6px, #111 6px 12px);
  background-size: 12px 1px, 12px 1px, 1px 12px, 1px 12px;
  background-position: 0 0, 0 100%, 0 0, 100% 0;
  background-repeat: repeat-x, repeat-x, repeat-y, repeat-y;
  animation: nhost-ss-ants 1.2s linear infinite;
}
@keyframes nhost-ss-ants {
  to {
